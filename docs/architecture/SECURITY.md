...
| Secrets hygiene (git‑ignored `.env.local`, committed `.env.example`) | `.env.local`, the assessment brief and the Postman collection/environment (real base URLs + credentials) are git‑ignored; the committed `.env.example` carries placeholders. | [`.gitignore`](../../.gitignore), [`.env.example`](../../.env.example) |
